...
function PortfolioTransactionsTable(props: { transactions: PortfolioTransactionEntry[] }) {
                ...
                <Table style={{ minWidth: 150 }} aria-label="a dense table">
                    <TableHead>
                        <TableRow sx={{ backgroundColor: ColorsEnum.coolgray8 }}>
                            {Object.keys(props.transactions[0]).map(
                                    ...
                                    if (col !== 'portfolio_id' && col !== 'uuid') {
                                        return (
                                            <StyledTableCell
                                                ...
                                                width={
                                                    100 /
                                                        Object.keys(props.transactions[0]).length +
                                                    '%'
                                                }
                                                key={`${col}_${index}`}
                                            >
                                ...
                                <StyledTableRow>
                                    {Object.keys(entry).map(col => {
                                        if (col !== 'portfolio_id' && col !== 'uuid') {
                                            let value =
                                                entry[
                                                    col as keyof PortfolioTransactionEntry
                                                ]?.toString();
                                            if (col === 'price_purchased' || col === 'comission')
                                                value = `$${value}`;
                                            return (
                                                <StyledTableCell
...
export default function PortfolioSidePanel() {
    ...
    return (
        <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
            <S.PortfolioSidePanelBody></S.PortfolioSidePanelBody>
            ...
            <PortfolioTransactionPopup
                show={show}
                setShow={setShow}
                setTransactions={setTransactionsHandler}
            />
        </div>
    );
}

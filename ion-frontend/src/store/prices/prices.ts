...
export const forexStreamStore = create((set) => ({
    ...
    setForexStream: (streamObject: ForexTableDataType) =>
        ...
        set((state: any) => {
            ...
            const prev: FormattedForexDataType = streamObject ? state.forexStream[streamObject.instrument] : null
            return (
                streamObject ? 
                { forexStream: { 
                    ...state.forexStream, 
                    [streamObject.instrument]: {
                        ...
                        bid_change: (prev ? (streamObject.closeoutBid - prev.closeoutBid) > 0 ? 1 : -1 : 0), 
                        ask_change: (prev ? (streamObject.closeoutAsk - prev.closeoutAsk) > 0 ? 1 : -1 : 0),
                    } 
                } } : 
                null 
        )}),

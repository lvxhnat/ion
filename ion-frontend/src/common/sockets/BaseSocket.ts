export default class BaseSocket {
	...
	constructor({ socketURL, socket, name}: {socketURL?: string, socket?: WebSocket, name?: string}) {
		if (!socketURL && !socket) throw new Error('Ensure that socket is initialised with either a url or an existing socket.');
		if (socketURL && socket) console.warn('Socket URL and Socket initialised. Prioritising initialised Socket connection.');

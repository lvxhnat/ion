...
function formatDateString(d: Date) {
    ...
    return `${zeroPad(d.getDate())}/${zeroPad(d.getMonth())}/${d.getFullYear()} ${zeroPad(d.getHours())}:${zeroPad(d.getMinutes())}`
}
...
export const addToolTip = (props: Required<CreateToolTipProps>) => {
    ...
    function mousemove(e: any) {
        ...
        if (d0 && d1) {
            ...
            focus.select("#tooltip-x-tracker")
                .attr("transform", `translate(${props.x(props.dataX[i])}, ${LINECHARTCONFIGS.DEFAULT_HEIGHT + 5})`)

            focus.select("#tooltip-x-tracker-text")
                .text(formatDateString(props.dataX[i]))
                .attr("transform", `translate(${props.x(props.dataX[i])}, ${LINECHARTCONFIGS.DEFAULT_HEIGHT + 14})`)
        }
    }

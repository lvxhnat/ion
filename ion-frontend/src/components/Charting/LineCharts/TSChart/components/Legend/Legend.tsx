...
function ColorBox(props: { color: string }) {
    return <div style={{ backgroundColor: props.color, width: '10px', height: '10px' }} />;
}
                    ...
                    {props.data.map((legendData: LegendDataType, index: number) => {
                        return (
                            <React.Fragment key={`legendFragment_${index}`}>
                                ...
                                >
                                    <StyledTableCell
                                        key={`${legendData.name}_${index}_color`}
                                        width="10%"
                                        isText={false}
                                    >
                                    ...
                                    <StyledTableCell
                                        isText
                                        key={`${legendData.name}_${index}_legendName`}
                                        width="85%"
                                    >
                                        {legendData.name}
                                    </StyledTableCell>
                                    <StyledTableCell
                                        key={`${legendData.name}_${index}_removeIcon`}
                                        width="5%"
                                    >
                                        <IconButton
                                            disableRipple
                                            sx={{ padding: 0 }}
                                            onClick={legendData.f}
                                        >
                                ...
                                {legendData.indicators.map((subLegendData, index: number) => {
                                    return (
                                        ...
                                        >
                                            <StyledTableCell
                                                width="10%"
                                                key={`${legendData.name}_${index}_sub_color`}
                                            >
                                            ...
                                            <StyledTableCell
                                                isText
                                                width="85%"
                                                key={`${legendData.name}_${index}_sub_legendName`}
                                            >
                                                {subLegendData.name}
                                            </StyledTableCell>
                                            <StyledTableCell
                                                width="5%"
                                                key={`${legendData.name}_${index}_sub_removeIcon`}
                                            >
                                                <IconButton
                                                    disableRipple
                                                    sx={{ padding: 0 }}
                                                    onClick={subLegendData.f}
                                                >
                                                    <RemoveRedEyeIcon sx={{ fontSize: '12px' }} />
                                                </IconButton>
                                            </StyledTableCell>

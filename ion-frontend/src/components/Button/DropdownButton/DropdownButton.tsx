...
export const FlexRow = styled('div')<{ alternate?: boolean }>(({ theme, alternate }) => ({
    gap: 5,
    display: 'flex',
...
export default function DropdownButton(props: {
    ...
    return (
        <div>
            <ButtonWrapper onClick={() => setMenu(!menu)}>{props.children}</ButtonWrapper>
            <div
                style={{
                    zIndex: 10,
                    position: 'absolute',
            ...
            >
                {props.options.map((option: DropdownButtonOptionProps, index: number) => {
                    return (
                        <FlexRow
